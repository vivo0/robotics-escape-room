...
struct unsupported_type : public std::exception {
  ...
  unsupported_type(const std::string &w, const std::string &t) {
    std::stringstream ss;
    ss << "Unsupported " << w << " type '" << t
       << "'. You may want to add it to meta/interfaces.txt and recompile the "
          "ROS2Interface plugin.";
    message = ss.str();
  }
 ...
 public:
  ...
  void onMainScriptAboutToBeCalled(int &out) {
    int stopSimulationRequestCounter =
        sim::getInt32Param(sim_intparam_stop_request_counter);
    bool doNotRun =
        sim::getBoolParam(sim_boolparam_rosinterface_donotrunmainscript);
    if (doNotRun > 0) {
  ...
  bool shouldProxyBeDestroyedAfterSimulationStop(int scriptID) {
    if (sim::getSimulationState() == sim_simulation_stopped) return false;
    int property = sim::getScriptInt32Param(scriptID, sim_scriptintparam_type);
#if SIM_PROGRAM_FULL_VERSION_NB <= 4010003
    if (property & sim_scripttype_threaded) property -= sim_scripttype_threaded;
#else
    if (property & sim_scripttype_threaded_old)
      property -= sim_scripttype_threaded_old;
#endif
    if (property == sim_scripttype_addonscript ||
        property == sim_scripttype_addonfunction ||
        property == sim_scripttype_customizationscript)
      return false;
    return true;
  }

  static void ros_imtr_callback(
      const sensor_msgs::msg::Image::ConstSharedPtr &msg,
      SubscriptionProxy *subscriptionProxy, Plugin *plugin) {
    if (msg->is_bigendian) {
      std::cerr << "ros_imtr_callback: error: big endian image not supported"
                << std::endl;
      return;
    }
    ...
    if (!imageTransportCallback(subscriptionProxy->topicCallback.scriptId,
                                subscriptionProxy->topicCallback.name.c_str(),
                                &in_args, &out_args)) {
      std::cerr << "ros_imtr_callback: error: failed to call callback"
                << std::endl;
      return;
    }
  }
  ...
  rclcpp::QoS get_qos(const std::optional<simros2_qos> &opt_qos) {
    if (!opt_qos.has_value()) return rclcpp::QoS{10};
  ...
  void createSubscription(createSubscription_in *in,
                          createSubscription_out *out) {
...
#include <sub_new.cpp>
    ...
    out->subscriptionHandle = subscriptionProxy->handle =
        subscriptionHandles.add(subscriptionProxy, in->_.scriptID);
  }

  void shutdownSubscription(shutdownSubscription_in *in,
                            shutdownSubscription_out *out) {
    SubscriptionProxy *subscriptionProxy =
        subscriptionHandles.get(in->subscriptionHandle);
...
#include <sub_del.cpp>
  ...
  void subscriptionTreatUInt8ArrayAsString(
      subscriptionTreatUInt8ArrayAsString_in *in,
      subscriptionTreatUInt8ArrayAsString_out *out) {
    SubscriptionProxy *subscriptionProxy =
        subscriptionHandles.get(in->subscriptionHandle);
...
#include <pub_new.cpp>
    ...
    out->publisherHandle = publisherProxy->handle =
        publisherHandles.add(publisherProxy, in->_.scriptID);
  }
...
#include <pub_del.cpp>
  ...
  void publisherTreatUInt8ArrayAsString(
      publisherTreatUInt8ArrayAsString_in *in,
      publisherTreatUInt8ArrayAsString_out *out) {
    PublisherProxy *publisherProxy = publisherHandles.get(in->publisherHandle);
...
#include <cli_new.cpp>
    ...
    out->clientHandle = clientProxy->handle =
        clientHandles.add(clientProxy, in->_.scriptID);
  }
...
#include <cli_del.cpp>
  ...
  void clientTreatUInt8ArrayAsString(clientTreatUInt8ArrayAsString_in *in,
                                     clientTreatUInt8ArrayAsString_out *out) {
    ClientProxy *clientProxy = clientHandles.get(in->clientHandle);
...
#include <srv_new.cpp>
    ...
    out->serviceHandle = serviceProxy->handle =
        serviceHandles.add(serviceProxy, in->_.scriptID);
  }
...
#include <srv_del.cpp>
  ...
  void serviceTreatUInt8ArrayAsString(serviceTreatUInt8ArrayAsString_in *in,
                                      serviceTreatUInt8ArrayAsString_out *out) {
    ServiceProxy *serviceProxy = serviceHandles.get(in->serviceHandle);
    serviceProxy->rd_opt.uint8array_as_string = true;
    serviceProxy->wr_opt.uint8array_as_string = true;
  }

  void createActionClient(createActionClient_in *in,
                          createActionClient_out *out) {
...
#include <actcli_new.cpp>
    ...
    out->actionClientHandle = actionClientProxy->handle =
        actionClientHandles.add(actionClientProxy, in->_.scriptID);
  }

  void shutdownActionClient(shutdownActionClient_in *in,
                            shutdownActionClient_out *out) {
    ActionClientProxy *actionClientProxy =
        actionClientHandles.get(in->actionClientHandle);
...
#include <actcli_del.cpp>
  ...
  void actionClientTreatUInt8ArrayAsString(
      actionClientTreatUInt8ArrayAsString_in *in,
      actionClientTreatUInt8ArrayAsString_out *out) {
    ActionClientProxy *actionClientProxy =
        actionClientHandles.get(in->actionClientHandle);
...
#include <actcli_cancelLastGoal.cpp>
  ...
  void createActionServer(createActionServer_in *in,
                          createActionServer_out *out) {
...
#include <actsrv_new.cpp>
    ...
    out->actionServerHandle = actionServerProxy->handle =
        actionServerHandles.add(actionServerProxy, in->_.scriptID);
  }

  void shutdownActionServer(shutdownActionServer_in *in,
                            shutdownActionServer_out *out) {
    ActionServerProxy *actionServerProxy =
        actionServerHandles.get(in->actionServerHandle);
...
#include <actsrv_del.cpp>
  ...
  void actionServerTreatUInt8ArrayAsString(
      actionServerTreatUInt8ArrayAsString_in *in,
      actionServerTreatUInt8ArrayAsString_out *out) {
    ActionServerProxy *actionServerProxy =
        actionServerHandles.get(in->actionServerHandle);
  ...
  rclcpp_action::ServerGoalHandle<Action> *getGoalHandle(
      ActionServerProxy *actionServerProxy, const std::string &goalUUIDstr) {
    return getGoalHandle<Action>(actionServerProxy,
                                 goalUUIDfromString(goalUUIDstr));
  }

  template <typename Action>
  rclcpp_action::ServerGoalHandle<Action> *getGoalHandle(
      ActionServerProxy *actionServerProxy,
      const rclcpp_action::GoalUUID &goalUUID) {
    auto actsrv =
        boost::any_cast<std::shared_ptr<rclcpp_action::Server<Action>>>(
            actionServerProxy->action_server);
    auto it = actionServerProxy->goalHandles.find(goalUUID);
    if (it == actionServerProxy->goalHandles.end())
      throw sim::exception("goal handle '%s' does not exist",
                           goalUUIDtoString(goalUUID));
    auto goalHandleBase = it->second.get();
    return dynamic_cast<rclcpp_action::ServerGoalHandle<Action> *>(
        goalHandleBase);
  }
  ...
  void cleanupTerminalGoalHandles(ActionServerProxy *actionServerProxy) {
    ...
    for (auto it = actionServerProxy->goalHandles.begin();
         it != actionServerProxy->goalHandles.end();) {
  ...
  void actionServerPublishFeedback(actionServerPublishFeedback_in *in,
                                   actionServerPublishFeedback_out *out) {
    ActionServerProxy *actionServerProxy =
        actionServerHandles.get(in->actionServerHandle);
...
#include <actsrv_action_publish_feedback.cpp>
  ...
  void actionServerActionAbort(actionServerActionAbort_in *in,
                               actionServerActionAbort_out *out) {
    ActionServerProxy *actionServerProxy =
        actionServerHandles.get(in->actionServerHandle);
...
#include <actsrv_action_abort.cpp>
  ...
  void actionServerActionSucceed(actionServerActionSucceed_in *in,
                                 actionServerActionSucceed_out *out) {
    ActionServerProxy *actionServerProxy =
        actionServerHandles.get(in->actionServerHandle);
...
#include <actsrv_action_succeed.cpp>
  ...
  void actionServerActionCanceled(actionServerActionCanceled_in *in,
                                  actionServerActionCanceled_out *out) {
    ActionServerProxy *actionServerProxy =
        actionServerHandles.get(in->actionServerHandle);
...
#include <actsrv_action_canceled.cpp>
  ...
  void actionServerActionExecute(actionServerActionExecute_in *in,
                                 actionServerActionExecute_out *out) {
    ActionServerProxy *actionServerProxy =
        actionServerHandles.get(in->actionServerHandle);
...
#include <actsrv_action_execute.cpp>
  ...
  void actionServerActionIsCanceling(actionServerActionIsCanceling_in *in,
                                     actionServerActionIsCanceling_out *out) {
    ActionServerProxy *actionServerProxy =
        actionServerHandles.get(in->actionServerHandle);
...
#include <actsrv_action_is_canceling.cpp>
  ...
  void actionServerActionIsActive(actionServerActionIsActive_in *in,
                                  actionServerActionIsActive_out *out) {
    ActionServerProxy *actionServerProxy =
        actionServerHandles.get(in->actionServerHandle);
...
#include <actsrv_action_is_active.cpp>
  ...
  void actionServerActionIsExecuting(actionServerActionIsExecuting_in *in,
                                     actionServerActionIsExecuting_out *out) {
    ActionServerProxy *actionServerProxy =
        actionServerHandles.get(in->actionServerHandle);
...
#include <actsrv_action_is_executing.cpp>
  ...
  void sendStaticTransform(sendStaticTransform_in *in,
                           sendStaticTransform_out *out) {
    geometry_msgs::msg::TransformStamped t;
    read__geometry_msgs__msg__TransformStamped(in->_.stackID, &t);
    stfbr->sendTransform(t);
  }

  void sendStaticTransforms(sendStaticTransforms_in *in,
                            sendStaticTransforms_out *out) {
  ...
  void getTransform(getTransform_in *in, getTransform_out *out) {
    try {
      geometry_msgs::msg::TransformStamped transform =
          tf_buffer->lookupTransform(in->target_frame, in->source_frame,
                                     tf2::TimePoint());
      ROS2WriteOptions wo;
      write__geometry_msgs__msg__TransformStamped(transform, in->_.stackID,
                                                  &wo);
    } catch (tf2::LookupException &ex) {
      RCLCPP_WARN(node->get_logger(), "Transform not available: %s", ex.what());
    } catch (tf2::ConnectivityException &ex) {
      RCLCPP_WARN(node->get_logger(), "Transform not available: %s", ex.what());
    }
  }

  void imageTransportCreateSubscription(
      imageTransportCreateSubscription_in *in,
      imageTransportCreateSubscription_out *out) {
#if image_transport_FOUND
    SubscriptionProxy *subscriptionProxy = new SubscriptionProxy();
    subscriptionProxy->topicName = in->topicName;
    ...
    subscriptionProxy->imageTransportSubscription = imtr->subscribe(
        in->topicName, in->queueSize,
        std::bind(ros_imtr_callback, _1, subscriptionProxy, this));

    if (!subscriptionProxy->imageTransportSubscription) {
      throw sim::exception(
          "failed creation of ROS ImageTransport subscription");
    }
...
#else
    throw sim::exception(
        "image_transport not available. please rebuild this plugin.");
#endif
  }

  void imageTransportShutdownSubscription(
      imageTransportShutdownSubscription_in *in,
      imageTransportShutdownSubscription_out *out) {
    SubscriptionProxy *subscriptionProxy =
        subscriptionHandles.get(in->subscriptionHandle);
  ...
  void imageTransportCreatePublisher(imageTransportCreatePublisher_in *in,
                                     imageTransportCreatePublisher_out *out) {
#if image_transport_FOUND
    PublisherProxy *publisherProxy = new PublisherProxy();
    publisherProxy->topicName = in->topicName;
    publisherProxy->topicType = "@image_transport";

    publisherProxy->imageTransportPublisher =
        imtr->advertise(in->topicName, in->queueSize);
  ...
  void imageTransportShutdownPublisher(
  ...
  void imageTransportPublish(imageTransportPublish_in *in,
                             imageTransportPublish_out *out) {
#if image_transport_FOUND
    PublisherProxy *publisherProxy = publisherHandles.get(in->publisherHandle);
  ...
  void getTime(getTime_in *in, getTime_out *out) {
    ...
    switch (in->clock_type) {
      case simros2_clock_ros:
        t = RCL_ROS_TIME;
        break;
      case simros2_clock_system:
        t = RCL_SYSTEM_TIME;
        break;
      case simros2_clock_steady:
        t = RCL_STEADY_TIME;
        break;
    }
  ...
  void deleteParam(deleteParam_in *in, deleteParam_out *out) {
    if (params_client->has_parameter(in->name))
      params_client->set_parameters(
          {rclcpp::Parameter(in->name, rclcpp::ParameterValue())});
  }
...
#include <if_new.cpp>
  ...
  void getInterfaceConstants(getInterfaceConstants_in *in,
                             getInterfaceConstants_out *out) {
    if (0) {
    }
#include <if_const.cpp>
  ...
  void supportedInterfaces(supportedInterfaces_in *in,
                           supportedInterfaces_out *out) {
#include <if_list.cpp>
  }
  ...
  bool initialize() {
    ...
    while (!params_client->wait_for_service(1s)) {
      if (!rclcpp::ok()) {
        sim::addLog(sim_verbosity_errors,
                    "Interrupted while waiting for parameters service");
        return false;
      }
      sim::addLog(sim_verbosity_debug,
                  "Parameters service not available. Waiting for it...");
    }
