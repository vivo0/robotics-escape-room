...
from gripper import close_gripper, open_gripper
...
def _action(ep_robot):
    ...
    for _ in range(1000):
        ep_robot.led.set_led(
            r=120, g=0, b=230, effect=robomaster.led.EFFECT_BREATH, freq=10
        )
        ep_robot.chassis.move(
            x=1, y=0, z=0, xy_speed=0.5, z_speed=45
        ).wait_for_completed()
        ep_robot.led.set_led(
            r=230, g=50, b=100, effect=robomaster.led.EFFECT_FLASH, freq=5
        )
        ep_robot.chassis.move(
            x=0, y=0, z=90, xy_speed=0.5, z_speed=45
        ).wait_for_completed()
