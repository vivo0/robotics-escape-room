...
\caption{System architecture in three layers: simulation (blue),
perception and state estimation (green), and mission control (purple).
Orange: Nav2 and slam\_toolbox (third-party). Arrows show ROS~2 topics,
TF transforms, and the ZMQ side-channel.}
\label{fig:arch}
\end{figure*}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{fig_camera_pov.png}}
\caption{Robot's camera view during exploration with HSV blob
detection overlays. Detected regions for the cube (magenta),
pressure plate (green), and door (blue). The centroid of each
blob is used to estimate the object's position.}
\label{fig:blob}
\end{figure}
...
The mission controller runs as a timer callback
