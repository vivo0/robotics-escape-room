...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{fig_scenarios.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{fig_camera_blob.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{fig_map.png}}
...
this tolerance is large enough for navigation to waypoints but too
loose for precision placement (see Section~\ref{sec:mission}).
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{fig_fsm.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{fig_stage_explore.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{fig_stage_pickup.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{fig_stage_drop.png}}
...
Nav2's 0.25~m goal-checker tolerance is too coarse for a 0.3~m plate.
...
The stock RoboMaster gripper script in CoppeliaSim calls
\texttt{sim.setObjectParent} to reparent any grasped object as a
child of the gripper's attach point.
With reparenting, the cube's orientation is locked to the gripper:
any yaw rotation during driving would tilt the cube and cause it to
miss the plate on release.
We blocked \texttt{setObjectParent} entirely by overriding it with a
no-op in a Lua shim that the scene builder injects into the gripper
script, and instead rely on friction contact alone to hold the cube.
To make this reliable we raised the friction coefficient to 1.0 on
the gripper finger shapes and the cube (across all supported physics
engines) and added linear and angular damping to the cube so it
does not bounce or spin when first squeezed.
